...
def classWrapper():
    '''
    depending on inputs format it will create a child class of one 
    '''
    pass
...
class Subject(object):
    def __init__(self, subject_file):
        ...
        self.raw = self.readBDF(subject_file,n_eeg=136,hpts="/home/shared/AM_EEG/data/biosemi128.hpts", preload=True) # create rawBDF instance
            
    def readBDF(self,bdf_file, n_eeg=None, stim_channel=-1, annot=None, annotmap=None,  hpts=None, preload=False, verbose=None):
        '''
        ...
        return RawBDF(input_fname=bdfname, n_eeg=n_eeg,stim_channel=stim_channel, annot=annot, annotmap=annotmap, hpts=hpts, preload=preload,verbose=verbose)
...
class RawBDF(mne.io.edf.edf.RawEDF):
    ...
    '''
    def __init__(self,input_fname, n_eeg=None, stim_channel=-1, annot=None, annotmap=None, hpts=None, preload=False, verbose=None):
        super(RawBDF,self).__init__(input_fname, n_eeg=n_eeg,stim_channel=stim_channel, annot=annot, annotmap=annotmap, hpts=hpts, preload=preload,verbose=verbose)
    ...
    def defineEOG(self,channels = list(xrange(128,132))):
        ...
        print 'EOG channels ({0},{1},{2},{3}) successfully defined as ({4},{5},{6},{7}) '.format(channels[0],channels[1],channels[2],channels[3], chan_translater.values()[0],chan_translater.values()[1],chan_translater.values()[2],chan_translater.values()[3])

    def redefineType(self, channels, type):
        '''
        Change channel type of passed channels to misc. So that no eeg anymore
        '''
        for chI, channel in enumerate(channels):
            self.info['chs'][self.ch_names.index(channel)]['kind'] = 502
    ...
    def runICA(self, n_components = 50, method = 'fastica', picks = None, decim = 3, enable_plots = False):
        """
        ...
        ica.fit(self.raw, picks=picks, decim=3)
        n_max_eog = 1
        for eog_channel in ['VEOG1','VEOG2','HEOG1','HEOG2']:
            eog_inds, scores = ica.find_bads_eog(self.raw, eog_channel)
            show_picks = np.abs(scores).argsort()[::-1][:5]
            #ica.plot_sources(self.raw, show_picks, exclude=eog_inds, title='eog')
        ...
        ica.apply(self.raw, copy=False)

#this script is only supposed to provide a raw file that is as clean as possible
def main(subj):
    subject = Subject(subj)
    subject.raw.rereference(['EXG5','EXG6'])
    #add EOG labels to respective channels
    subject.raw.defineEOG()
    subject.raw.redefineType(['EXG7','EXG8'] , type = 502)
    #if necessary swap replaced electrodes 
    if subject.replacements != [()]:
        subject.raw.replaceElecs(subject.replacements)

    subject.raw.info['bads'] = subject.raw.info['bads'] + subject.bad_channels + ['EXG7','EXG8'] 
    good_eeg = mne.pick_types(subject.raw.info, meg = False, eeg = True, stim = False, eog = False, exclude = 'bads')

    # save pure raw
    if not createDir(subject.data_path, subject.ID+'_basic_raw.fif', 'check' ):
        subject.raw.save(os.path.join(subject.data_path,subject.ID+'_basic_raw.fif'))
    
    # load raw anew as fif (otherwise some functions won't work, e.g. ICA)
    #subject.raw = mne.io.Raw(os.path.join(subject.data_path,subject.ID+'_basic_raw.fif'),preload = True) 

    #filter
    subject.raw.filter(l_freq = 1, h_freq = 100)

    #subject.raw.plot(start = 24)

    '''
    ica = mne.preprocessing.ICA(n_components=20, method='fastica')
    ica.fit(subject.raw, picks=good_eeg, decim=3)
    n_max_eog = 1
    for eog_channel in ['VEOG1','VEOG2','HEOG1','HEOG2']:
        eog_inds, scores = ica.find_bads_eog(subject.raw, eog_channel)
        show_picks = np.abs(scores).argsort()[::-1][:5]
        ica.plot_sources(subject.raw, show_picks, exclude=eog_inds, title='eog')
        ica.exclude += eog_inds
    ica.apply(subject.raw, copy=False)
    #ica
    #subject.raw.run_ica()
    subject.raw.plot(start = 24)
    '''
    subject.raw.runICA(picks= good_eeg)
    
    # save postICA raw
    if not createDir(subject.data_path, subject.ID+'_postICA_raw.fif', 'check' ):
        subject.raw.save(os.path.join(subject.data_path,subject.ID+'_postICA_raw.fif'))
    shell()

    subject.raw.events = mne.find_events(subject.raw)
    # do Laplace
    # artefact rejection/correction?
    event_dict = dict(on_am = 1)
    direction_epochs = eu.makeEpochs(subject.raw,subject.raw.events, good_eeg, -1.5,2.5, event_type = ['on_am'])
    ...
    plot1.savefig('/home/shared/AM_EEG/results/'+subject.ID+'_pre_laplace.pdf', format = 'pdf')    


    """
    #G,H, cosdist = subject.raw.LaplacianTransform(picks = good_eeg, m=4,legendre_order = 20,lamda = 10e-5)
    # save raw after laplace
    if not createDir(subject.data_path, subject.ID+'_laplace_raw.fif', 'check' ):
        subject.raw.save(os.path.join(subject.data_path,subject.ID+'_m'+str(m)+'_order'+str(order)+'_lamda'+ str(lamda)+'_laplace_raw.fif'))
    ...
    #plot2.savefig('/home/shared/AM_EEG/results/'+subject.ID+'_m'+str(m)+'_order'+str(order)+'_lamda'+ str(lamda)+'_post_laplace.pdf', format = 'pdf')
