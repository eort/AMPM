...
class RawBDF(mne.fiff.edf.edf.RawEDF):
    '''
    ...
    def __init__(self,input_fname, n_eeg=None, stim_channel=-1, annot=None, annotmap=None, hpts=None, preload=False, verbose=None):
        ...
        logging.info('Raw instance was created for subject {0}'.format(input_fname[-8:]))
    ...
    def run_ica(self, n_components = 0.95, method = 'fastica', picks = None, decim = 3, enable_plots = False):
        """
...
def main(subj):
    ...
    # save pure raw
    if not createDir(subject.data_path, subject.ID+'_unprocessed_raw.fif', 'check' ):
        subject.raw.save(os.path.join(subject.data_path,subject.ID+'_unprocessed_raw.fif'))
    
    # load raw anew as fif (otherwise some functions won't work, e.g. ICA)
    subject.raw = mne.io.Raw(os.path.join(subject.data_path,subject.ID+'_unprocessed_raw.fif'),preload = True) 
